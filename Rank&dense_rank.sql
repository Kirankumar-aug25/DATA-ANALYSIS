...
#============================ROWNumber================================#
#must and should order by
#partition by is optional for row number and row number will start with number
 find the 5th highest salary 
select * from
(select first_name,department,salary,row_number() over (order by salary desc) as ROWNumber
from worker)temptable;
#=====================================================================#
SELECT * from
(select first_name,department,salary,row_number() over (partition by department order by salary desc ) as ROWNumber
 from worker)temptable;
#=======================rank&dense_rank================================#
select first_name,department,salary,rank() over (order by salary desc) as r_nk
from worker;
select first_name,department,salary,dense_rank() over (partition by department order by salary desc) as r_nk
from worker;
